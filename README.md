...
| [`exposicion-larga.html`](exposicion-larga.html) | Exposición larga: las seis técnicas de la chuleta de Pit Haupert con el tiempo que piden en tu escena, no el de la receta. Calcula el arrastre del sujeto en píxeles (b = f·v·t/(d·p)), si lo sostienes a pulso con la escalera de apoyos (codos, correa tensada, apoyo, OIS, IBIS, trípode), qué filtro hace falta y qué ISO sin él. Con franja del sensor a escala, curva de arrastre frente a trepidación y comparación de los tres cuerpos. | 64 KB |
...
| [`exposicion-larga.html`](exposicion-larga.html) | Long exposure: the six techniques of Pit Haupert's cheat sheet with the time they ask for in your scene, not the recipe's. Computes subject blur in pixels (b = f·v·t/(d·p)), whether you can hold it with the ladder of supports (elbows, tensioned strap, bracing, OIS, IBIS, tripod), which filter you need and what ISO without one. With a to-scale sensor strip, a blur-against-shake curve and a three-body comparison. | 64 KB |
