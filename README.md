...
|---|---|---:|
| [`simulador-eclipse.html`](simulador-eclipse.html) | Encuadre y exposición del eclipse total del 2 de agosto de 2027 (sur de España, norte de Marruecos, Egipto): si la corona te cabe en el encuadre, horquilla por fases con la fotometría de Espenak, nueve sitios de Tarifa a Luxor con sus horas locales, y ficha de campo imprimible. | 186 KB |
| [`simulador-flash.html`](simulador-flash.html) | Banco óptico de flash. La dureza de una sombra la fija el tamaño angular de la fuente vista desde el sujeto: sombras, caída de luz por número guía, vencer al sol y balance con la luz ambiente. | 97 KB |
| [`exposicion-larga.html`](exposicion-larga.html) | Exposición larga: las seis técnicas de la chuleta de Pit Haupert, y una séptima (cámara quieta, el sujeto deja traza), con el tiempo que piden en tu escena, no el de la receta. Calcula el arrastre del sujeto en píxeles (b = f·v·t/(d·p)), si lo sostienes a pulso con la escalera de apoyos (codos, correa tensada, apoyo, OIS, IBIS, trípode), qué filtro hace falta y qué ISO sin él. Con el sujeto (persona, ciclista, coche, tren) dibujado a su tamaño real en el fotograma y la traza que deja, curva de arrastre frente a trepidación y comparación de los tres cuerpos. | 80 KB |
| [`programa-camara.html`](programa-camara.html) | La automática no es neutral: contiene un programa. Dibuja la línea que sigue —tiempo, diafragma e ISO a cada nivel de luz—, superpone el borrón y la profundidad de campo que tu intención exigía, y señala dónde chocan. Nace de la sección Flusser del ensayo del marco. | 107 KB |
| [`niebla.html`](niebla.html) | Si la niebla se va a formar mañana al amanecer. Índice horario a 48 h sobre la convergencia de temperatura y punto de rocío, y una fusión bayesiana multimodelo —ahora con conjuntos ICON EPS, GFS ENS y ECMWF ENS de 30 a 50 miembros— que reajusta sus pesos con tus propias observaciones y se recalibra (Platt) con tu registro. La previsión se congela automáticamente cada noche, así que la observación de la mañana se compara con lo que se predijo la víspera y no con nada posterior. Necesita conexión: los datos son de Open-Meteo. Manual: [`manual-niebla.html`](manual-niebla.html); guía rápida: [`guia-niebla.html`](guia-niebla.html). | 30 KB |
...
| Documento | Qué hace | Peso |
|---|---|---:|
| [`analisis-centrado.html`](analisis-centrado.html) | Dónde cae el sujeto dentro del fotograma a lo largo de una colección entera, con qué dispersión y con qué sesgo. Contrasta tu práctica real con la regla de los tercios y con el centrado estricto, sin dar por buena ninguna. Salida visual de [`subject_center.py`](scripts/analitica/subject_center.py). | 214 KB |
| [`equipo-real.html`](equipo-real.html) | Arrastra una carpeta de fotos y lee su EXIF en el navegador, sin subir nada: focales, diafragmas, ISO, velocidades, EV de escena y horas por cuerpo y por objetivo, con lector propio sin dependencias (JPEG, TIFF y DNG). Veredictos con los umbrales de las otras fichas —difracción por cuerpo, trepidación 1/f, ganancia dual—, mapa focal × diafragma, cobertura de una focal o dos y comparación entre periodos. La distancia entre lo que crees que usas y lo que usas. | 101 KB |
...
| [`explorador_distancias.html`](explorador_distancias.html) | 292 autores colocados por parecido entre sus imágenes, con las fotos incrustadas. Da la distancia de contenido de cada pareja y, a la vez, su distancia en la red de influencia. | 6,4 MB |
...
| [`scripts/sitio/`](scripts/sitio/) | `fusionar.py`, el constructor con el que se han fusionado páginas en una sola con pestañas (aísla el CSS de cada fuente, renombra los ids que chocan, encadena los conmutadores de idioma), las especificaciones de `sensor.html` y `filtro-sensor.html`, y en `fuentes/` las páginas originales que se fusionan (las cinco antiguas más `parte-difraccion.html`, la pestaña de difracción escrita ya como parte). Para reconstruir: `python3 scripts/sitio/fusionar.py scripts/sitio/fusion-sensor.json`. |
...
| [`exposicion-larga.html`](exposicion-larga.html) | Long exposure: the six techniques of Pit Haupert's cheat sheet, plus a seventh (still camera, the subject leaves a trace), with the time they ask for in your scene, not the recipe's. Computes subject blur in pixels (b = f·v·t/(d·p)), whether you can hold it with the ladder of supports (elbows, tensioned strap, bracing, OIS, IBIS, tripod), which filter you need and what ISO without one. With the subject (person, cyclist, car, train) drawn at its true size in the frame together with the trace it leaves, a blur-against-shake curve and a three-body comparison. | 80 KB |
...
| [`equipo-real.html`](equipo-real.html) | Drop a folder of photographs and read their EXIF in the browser, uploading nothing: focal lengths, apertures, ISO, shutter speeds, scene EV and hours per body and lens, with a dependency-free reader of its own (JPEG, TIFF and DNG). Verdicts using the thresholds of the sibling pages —diffraction per body, 1/f camera shake, dual gain—, a focal × aperture map, coverage of one focal length or two, and a comparison between periods. The gap between what you think you use and what you use. | 101 KB |
...
| [`manual-siril-apilado.html`](manual-siril-apilado.html) | Siril step by step for stacking deep sky with the M11P and a front UHC filter: capture, calibration with flats taken with the filter on, registration, Winsorized-rejection stacking and postprocessing (SPCC, GHS, StarNet++), with the interface map, the click-path equivalent of every command, the no-darks variant for a modern CMOS, progress checkboxes and a red night mode. | 82 KB |
...
| [`explorador_distancias.html`](explorador_distancias.html) | 292 authors placed by the likeness between their images, with the photographs embedded. Gives the content distance for each pair and, alongside it, their distance in the influence network. | 6.4 MB |
...
| [`scripts/sitio/`](scripts/sitio/) | `fusionar.py`, the builder used to merge pages into a single tabbed one (isolates each source's CSS, renames colliding ids, chains the language switchers), the specs for `sensor.html` and `filtro-sensor.html`, and in `fuentes/` the original pages being merged (the five old ones plus `parte-difraccion.html`, the diffraction tab written directly as a part). To rebuild: `python3 scripts/sitio/fusionar.py scripts/sitio/fusion-sensor.json`. |
...
| [`paseos/flash.html`](paseos/flash.html) | 4 · Flash | Las dos exposiciones que conviven en la foto, el número guía, la sincronización, el tamaño de la fuente, el rebote y el relleno a la luz del día; seis fotógrafos del flash (Weegee, Gilden, Parr, diCorcia, Goldin, Fink); banco de flash con sombra, histograma y reparto flash/ambiente. | 223 KB |
